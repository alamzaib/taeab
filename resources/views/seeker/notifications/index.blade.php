...
<div class="container" style="max-width:1200px; margin:40px auto;">
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
        ...
        @if($notifications->where('is_read', false)->count() > 0)
            <form action="{{ route('seeker.notifications.read-all') }}" method="POST" style="display:inline;">
                @csrf
                        ...
                        <div style="flex:1;">
                            ...
                            <div style="display:flex; gap:16px; font-size:13px; color:#6b7280;">
                                ...
                                @if($notification->jobApplication)
                                    <a href="{{ route('seeker.applications.show', $notification->jobApplication) }}" style="color:#235181; text-decoration:none;">View Application</a>
                                @endif
                            </div>
                        </div>
                        @if(!$notification->is_read)
                            <form action="{{ route('seeker.notifications.read', $notification) }}" method="POST" style="display:inline;" class="mark-read-form">
                                @csrf
