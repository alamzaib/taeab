@php
    $allApplications = \App\Models\JobApplication::with(['job', 'seeker'])
...
<p style="color:#64748b; margin-bottom:24px;">Communicate with job seekers about their applications.</p>
                ...
                <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:16px; flex-wrap:wrap;">
                    <div style="flex:1;">
                        ...
                        <p style="margin:0 0 8px; color:#64748b;">
                            Applicant: <strong>{{ $application->seeker->name }}</strong>
                        </p>
                        @if($latestMessage)
                    ...
                    <div>
                        <button class="btn btn-primary open-chat-btn"
                            ...
                            data-seeker-name="{{ $application->seeker->name }}">
    ...
    <div class="chat-modal-container">
        <div class="chat-modal-header">
            <div>
                ...
                <p id="chatSeekerName" style="margin:4px 0 0; color:#64748b; font-size:14px;"></p>
            </div>
...
.chat-message-seeker .chat-message-time {
    text-align:left;
}
</style>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.open-chat-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const seekerName = this.getAttribute('data-seeker-name');
            openChatModal(applicationId, jobTitle, seekerName);
        });
    });
});

function openChatModal(applicationId, jobTitle, seekerName) {
    currentApplicationId = applicationId;
    document.getElementById('chatJobTitle').textContent = jobTitle;
    document.getElementById('chatSeekerName').textContent = seekerName;
    ...
    .then(data => {
        ...
        if (data.messages && data.messages.length > 0) {
            chatBody.innerHTML = data.messages.map(msg => {
                ...
                return `
                    <div class="chat-message ${isCompany ? 'chat-message-company' : 'chat-message-seeker'}">
                        <div>
...
function sendChatMessage(event) {
    ...
    fetch(`/company/applications/${currentApplicationId}/messages`, {
        method: 'POST',
        body: formData,
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
